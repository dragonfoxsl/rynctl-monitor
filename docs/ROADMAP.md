...
> Last updated: 2026-07-24. Statuses describe the current tree, not intent from
> the original March plan.
...
| 1.5 | **Mobile responsive layout** — Responsive CSS so the app is usable on tablets and phones; collapsible sidebar, stacked tables on small screens | CSS media queries + layout refactor | Pending; current test plan treats sub-900px layout as a gap |
...
| 2.4 | **Dark / light theme toggle** — User-selectable theme with persistence; dark theme is current default | CSS variables + persisted frontend state | Implemented and covered by Playwright |
...
| # | Feature | Scope | Status |
|---|---------|-------|--------|
| 4.1 | **Expand Playwright E2E tests** — Broaden UI/API coverage beyond login, job CRUD, theme, accessibility, and scheduler smoke | `tests/e2e/` specs | In progress; core flows run in CI, but the full matrix listed here is not complete |
| 4.2 | **Python pytest unit tests** — API and backend regression coverage | `tests/unit/` with pytest + httpx | Implemented and run in CI; further coverage remains ongoing |
| 4.3 | **CI pipeline** — Test, audit, build, Playwright, and scheduled-job smoke on push/PR | `.github/workflows/ci.yml` | Implemented; no Ruff/ESLint jobs are currently configured |
...
- Configurable run/log retention (`RYNCTL_RUN_RETENTION_DAYS`)
