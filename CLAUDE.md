...
make up                             # start app (rynctl-monitor service only)
make down                           # stop
...
| `middleware.py` | SecurityHeadersMiddleware, RateLimitMiddleware (per-IP sliding window), CSRFMiddleware, RequestLoggingMiddleware |
...
| `lib/flags.js` | 15 rsync flags with descriptions, DEFAULT_FLAGS: [-a, -v, -h] |
...
| `pages/Crontab.jsx` | System crontab viewer — lists scanned rsync lines, import as jobs |
| `pages/Flags.jsx` | Read-only reference describing every rsync flag from `lib/flags.js` |
...
Schema auto-applied on first startup. New columns added via `ALTER TABLE IF NOT EXISTS` pattern in `database.py`. Admin user seeded if DB is empty, password from `RYNCTL_ADMIN_PASSWORD` (defaults to `admin`).
...
| `RYNCTL_RUN_RETENTION_DAYS` | `0` | Prune job_runs + logs older than N days (0=keep forever) |
...
- Middleware order: SecurityHeaders → RequestLogging → RateLimit → CSRF (outermost first)
- Security headers (X-Content-Type-Options, X-Frame-Options, Referrer-Policy, CSP) on all responses; HSTS when `RYNCTL_SECURE_COOKIES=true`
...
- Commit messages must not mention Claude, AI assistants, or include co-author/"generated by" trailers
